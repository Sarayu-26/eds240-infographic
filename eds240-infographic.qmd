...
downloaded vessel watch data ais from years 2017-2024 and chnms shapefile boundary.
...
fleet_chnms <- lapply(file_list, gfw_one_file) %>%
  bind_rows()
...
EDA spatial view: preliminary af just to see if the plots are plotted and what they look like.

Now when we are working with the data we can pretty much start from here.
...
ggplot() +
  ...
  theme_minimal() +
  labs(title = "Fishing points inside CHNMS (2017–2024)")
