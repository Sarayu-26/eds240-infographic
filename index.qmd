...
## Fishing Activity in Chumash Heritage National Marine Sanctuary: Insights from AIS Data

### Spawning Idea

I’ve been exploring fishing activity in Chumash Heritage National Marine Sanctuary, which stretches along the central California coast from Gaviota to just south of Morro Bay and protects a variety of nearshore and offshore marine habitats. This sanctuary includes kelp forests, rocky reefs, and open-water areas, providing homes for fish, invertebrates, and marine mammals. I wanted to better understand human interactions within the sanctuary, especially fishing because it’s a key piece of understanding how we can engage and protectthese ecosystems.

Understanding preliminary fishing activity in the area was a little tricky. The sanctuary sits between two active harbors: Morro Bay Harbor to the north and Santa Barbara Harbor to the south. which makes it hard (for the average public data parser and fisheries amateur!) to know exactly what’s being caught and where. Catch landing data is reported at the harbor level, not the sanctuary level, so I couldn't tell whether a fish and which gear type used was caught inside the sanctuary or somewhere outside and just landed nearby.

To get around this, I looked at vessel tracking data from [Global Fishing Watch]{.underline} (GFW). They collect Automatic Identification System (AIS) signals from vessels, which broadcast location, speed, and heading. Many large commercial vessels are required to carry AIS (\>65ft long), and some smaller vessels do so voluntarily. Using AIS data lets us get an idea of where and when fishing vessels are active, even if catch records are limited. Of course, there are caveats: many smaller vessels don’t carry AIS, and this data doesn’t include recreational, cultural, or subsistence fishing. Still, it gives me a useful glimpse into fishing vessel presence in the sanctuary.

**So the data...**

**AIS-Detected Fishing (2017–2024)**

For [Chumash Heritage National Marine Sanctuary]{.underline}, AIS-detected fishing data from **2017–2024** provide a snapshot of fishing activity within and around the sanctuary. This information helps illustrate patterns of ocean use and the presence of industrial fishing vessels over time.
...
![](figures/ais_ridgeline_affinity.png)
...
![](figures/canva_ais_circular_plot.png)

**These are the gear types used within the Sanctuary (Gear Type Context)**

So readers can understand what these gears target and where:

-   **Pole and line, trollers** : tuna and other offshore pelagic species

-   **Purse seines** : nearshore schooling fish like sardines and anchovies.

-   **Other purse seines** – offshore schooling species such as tuna.

-   **Drifting longlines** : offshore species like swordfish and tuna

-   **Set longlines, fixed gear** : bottom-associated fish such as rockfish and halibut

-   **Trawlers**: groundfish and shrimp

-   **Set gillnets** : halibut and white seabass

-   **Pots and traps** : crab and lobster
...
![](figures/AIS-%20Infographic-Draft-Canva.png)

### What are we seeing?

Together, these visualizations give a broader picture of AIS-detected fishing activity within [Chumash Heritage National Marine Sanctuary]{.underline}.
...
-   **Temporal Representation**: The ridgeline plot captures the timing of fishing activity across months and gears, showing both year-round and seasonal peaks.
    ...
    **Spatial Representation**: The density map communicates hotspots and nearshore/offshore patterns. I highlighted areas like Point Conception and Arguello Canyon/Terrace, which are present in the density map.
...
fleet_agg_gear_month <- fleet %>%
  ...
  summarise(total_gear_hours = sum(hours, na.rm = TRUE), .groups = "drop") %>%
  mutate(geartype_label = fct_reorder(geartype_label, total_gear_hours, .desc = TRUE))
...
# -------------------------------------------------------------------
# Define NOAA Themed color palette 
# -------------------------------------------------------------------
noaa_dark <- "#0D1B2A"
noaa_mid  <- "#5A8CAE"
noaa_pale <- "#D6EAF8"
...
ais_ridgeline <- ggplot(fleet_agg_gear_month,
  ...
  theme(
    ...
    legend.title           = element_text(family = "merriweather", face = "bold", size = 12),
