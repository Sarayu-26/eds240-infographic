...
#mean gear hour per individual gear type (over active years)
...
fleet_agg_gear <- fleet %>%
  filter(geartype_label != "Fishing") %>%
  group_by(geartype_label) %>%
  summarise(total_fishing_hours = sum(hours, na.rm = TRUE), 
            .groups = "drop") %>%
...
#aggregate gear type by season (ridgeline prep)
 fleet_agg_gear_month <- fleet %>%
  ...
  summarise(total_gear_hours = sum(fishing_hours, na.rm = TRUE), 
            .groups = "drop") %>%
  mutate(geartype_label = fct_reorder(geartype_label, 
                                      total_gear_hours, 
                                      .desc = TRUE))
```
...
ais_ridgeline <- ggplot(fleet_agg_gear_month,
  ...
  scale_fill_gradientn(
    ...
    guide = guide_colorbar(
      ...
      barwidth   = unit(0.755, "npc"),      # stretches across x-axis width
      barheight    = unit(0.4, "cm"),
      title.position = "top",
      ticks        = FALSE,               # clean, no tick marks on bar
      label        = FALSE                # month labels on x-axis serve this role
    )
  ) +
  labs(
    title    = "Seasonal AIS-detected fishing effort by gear type in CHNMS",
    subtitle = "Taller peaks indicate greater fishing density.",
    x        = NULL,
    y        = NULL,
  ...
  theme(
    ...
    legend.key.width       = unit(1, "npc"),# ← moved to bottom
    legend.margin    = margin(t = -15, r = 0, b = 0, l = 0), # ← pull it up closer to x-axis
    panel.spacing    = unit(0.1, "lines"),
    text             = element_text(family = "sourcesans"),
    plot.title       = element_text(
      family = "sourcesans", size = 25, face = "bold",
      color = "#0D1B2A", hjust = 0, margin = margin(b = 3)
    ),
    plot.subtitle    = element_text(
      family = "merriweather", size = 14,
      color = "#5A6670", hjust = 0, margin = margin(b = 20)
    ),
    ...
    axis.text.y      = element_text(
      color = "#0D1B2A", size = 16, face = "bold",
      hjust = 1, margin = margin(r = 10)
    ),
    axis.text.x      = element_text(
      color = "#5A6670", size = 13, margin = margin(t = 5)
    ),
    plot.margin      = margin(25, 25, 5, 25),
    legend.title     = element_text(
      family = "merriweather", face = "bold", size = 12
    ),
    legend.text      = element_text(family = "merriweather", size = 10)
  )
...
#save pdf 
# ggsave(
#   filename = here::here("figures", "ais_ridgeline.pdf"),
#   plot     = ais_ridgeline,
#   device   = cairo_pdf,
#   width    = 15,
#   height   = 10,
...
ggplot() +
  ...
  stat_density_2d(
    ...
    aes(x = cell_ll_lon, y = cell_ll_lat,
        weight = fishing_hours,
        fill = after_stat(level),
        alpha = after_stat(level)),
    geom = "polygon",
    contour = TRUE
  ) +
  geom_sf(data = california, fill = "gray90", color = "gray50") +  # mask density on land
  geom_sf(data = boundary, fill = NA, color = "black") +
  scale_fill_viridis_c(option = "viridis", direction = -1, name = "Fishing Effort\nDensity") +
...
density_plot <- ggplot() +
  ...
  geom_hex(data = fleet_ocean, aes(x = cell_ll_lon, y = cell_ll_lat, weight = fishing_hours),
              bins = 40, alpha = 0.8) +
...
density_plot <- ggplot() +
  ...
  geom_sf(data = california, fill = "gray90", color = "gray50") +  # drawn second to mask density on land
  geom_sf(data = boundary, fill = NA, color = "blue", size = 1) +
  scale_fill_viridis_c(option = "viridis", direction = -1, name = "Fishing Effort\nDensity") +
  scale_alpha(range = c(0.2, 0.9), guide = "none") +
  labs(
    title = "AIS-Vessel Fishing Effort Density in Chumash Heritage National Marine Sanctuary",
    subtitle = "Fishing effort is highly concentrated off of Point Conception.",
    caption = "Data Source: Global Fishing Watch (2017-2024)"
  ) +
  theme_void() +
  theme(
...
library(stringr)
  ...
  # Bars colored by magnitude gradient
 geom_col(
    aes(
      x = geartype_label, 
      y = total_fishing_hours, 
      fill = total_fishing_hours),
    position = "dodge2", 
    show.legend = TRUE, 
    alpha = 0.9, 
    width = 0.6
  ...
  # Lollipop shaft to per-gear mean
   geom_segment(
    aes(
      x = geartype_label, 
      xend = geartype_label, 
  ...
  theme(
    ...
    plot.caption     = element_text(
      ...
      color     = "#888888", hjust = 0,    # ← left align for long caption
      margin    = margin(t = 8)
    ),
    plot.margin      = margin(25, 25, 15, 25),
    legend.title     = element_text(
    ...
    axis.text.x      = element_text(
      family     = "merriweather", size = 35,
      color      = "#0D1B2A", lineheight = 0.3,
      face       = "bold"
    ),
    axis.text.y      = element_text(
      family = "merriweather", size = 18,
      color  = "gray30"
    )
  )
...
Alt Text: Set Gillnets and Pots & Traps are present in the sanctuary but not detected as actively fishing by GFW's algorithm, likely due to the passive nature of these gear types

### Bar Chart of Circular Bar Chart

```{r}
bar_plot <- ggplot(fleet_agg_gear) +
  ...
  trans  = "log1p",
  labels = comma,
  breaks = c(0, 25, 50, 100, 250, 500, 1000, 2000),
  ...
  theme(
    ...
    axis.title.y       = element_text(
      family = "merriweather", size = 45,
      color  = "gray30", margin = margin(r = 10)
    )
  )
...
ggsave(
  filename = here::here("figures", "ais_barplot.png"),
  plot = bar_plot,
  device = "png",
  width = 14,
  height = 8,
...
Total hours reflect AIS-detected vessel presence. GFW's fishing detection algorithm may undercount passive gear types such as pots & traps and set gillnets. Dashed line indicates mean annual vessel hours over active years per gear type.Y-axis is log-scaled
